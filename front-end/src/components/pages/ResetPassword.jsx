   ...
   const [data,setData] = useState({
    ...
    const handleSubmit = async(e) =>{
        ...
        if (data === ""){
          ...
          console.log(id)
          const res = await axios.post(`http://localhost:4532/blogs/reset-password/${id}`,data);
          setData({
            email:"",
            password:'',
             password2:''})
          if(res.status ===200){
            toast.success(" Your Password resseted successfully ")  
            navigate('/login'); 
             }
  ...
  return (
        ...
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            <div className="w-full p-6  bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md dark:bg-gray-800 dark:border-gray-700 sm:p-8">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4 lg:mt-5 md:space-y-5" method="post" action="/reset-password/<%= id %>/<%= token %>">
                    <div>
